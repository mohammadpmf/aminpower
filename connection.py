...
class Connection():
    def __init__(self, host='127.0.0.1', username='root', password='root'):
        ...
        query = "CREATE TABLE IF NOT EXISTS `amar2`.`tbl_users` (`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `name` VARCHAR(64) NOT NULL, `surname` VARCHAR(64) NOT NULL, `username` VARCHAR(64) NOT NULL, `password` VARCHAR(128) NOT NULL, `access_level` TINYINT(1) NOT NULL DEFAULT 2, `wrong_times` TINYINT(2) NOT NULL DEFAULT 0, `default_date` VARCHAR(64) NOT NULL DEFAULT 'روز قبل', PRIMARY KEY (`id`), UNIQUE INDEX `username_UNIQUE` (`username` ASC)) ENGINE = MyISAM;"
        self.cursor.execute(query)
        query = "CREATE TABLE IF NOT EXISTS `amar2`.`tbl_sections` (`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `title` VARCHAR(45) NOT NULL, `order` INT UNSIGNED NOT NULL, PRIMARY KEY (`id`), UNIQUE INDEX `title_UNIQUE` (`title` ASC)) ENGINE = MyISAM;"
        self.cursor.execute(query)
        query = "CREATE TABLE IF NOT EXISTS `amar2`.`tbl_places` (`id` INT UNSIGNED NOT NULL AUTO_INCREMENT, `title` VARCHAR(45) NOT NULL, `section` INT UNSIGNED NOT NULL, `order` INT UNSIGNED NOT NULL, PRIMARY KEY (`id`), INDEX `section_idx` (`section` ASC) , UNIQUE INDEX `place_section` (`title` ASC, `section` ASC) , CONSTRAINT `section` FOREIGN KEY (`section`) REFERENCES `amar2`.`tbl_sections` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT) ENGINE = MyISAM;"
        self.cursor.execute(query)
    ...
    def create_user(self, name, surname, username, password):
        try:
            query = "SELECT `id` FROM `amar2`.`tbl_users` ORDER BY `id` DESC LIMIT 1;"
            self.cursor.execute(query)
            ...
            query = "INSERT INTO `amar2`.`tbl_users` (`id`, `name`, `surname`, `username`, `password`) VALUES (%s, %s, %s, %s, %s);"
            values = auto_increament, name, surname, username, password
            self.cursor.execute(query, values)
            self.connection.commit()
            query = "SELECT `name`, `surname`, `username`, `password`, `access_level`, `wrong_times`, `default_date`, `id` FROM `amar2`.`tbl_users` where username=%s;"
            values = (username, )
            self.cursor.execute(query, values)
    ...
    def update_users_password(self, password):
        ...
        password = hash_password(password, salt)
        query = "UPDATE `amar2`.`tbl_users` SET `password` = %s WHERE (`id` = %s);"
    ...
    def update_wrong_times(self):
        query = "UPDATE `amar2`.`tbl_users` SET `wrong_times` = %s WHERE (`id` = %s);"
    ...
    def update_default_date_of_user(self):
        query = "UPDATE `amar2`.`tbl_users` SET `default_date` = %s WHERE (`id` = %s);"
    ...
    def create_part(self, title):
        query = "INSERT INTO `amar2`.`tbl_sections` (`title`, `order`) VALUES (%s, 0);"
        values = (title, )
        try:
            self.cursor.execute(query, values)
            self.connection.commit()
            query = "SELECT * FROM `amar2`.`tbl_sections` where title=%s;"
            self.cursor.execute(query, values)
            result = self.cursor.fetchone()
            if result in [None, '', ()]:
    ...
    def update_part(self, id, new_name):
        query = "UPDATE `amar2`.`tbl_sections` SET `title` = %s WHERE (`id` = %s);"
        values = (new_name, id)
        try:
            self.cursor.execute(query, values)
            self.connection.commit()
    ...
    def update_part_sort(self, id, order):
        query = "UPDATE `amar2`.`tbl_sections` SET `order` = %s WHERE (`id` = %s);"
        values = (order, id)
        self.cursor.execute(query, values)
        self.connection.commit()
        return ("ok", 0)
    ...
    def delete_part(self, id):
        query = "DELETE FROM `amar2`.`tbl_sections` WHERE (`id` = %s);"
        values = (id, )
        try:
            self.cursor.execute(query, values)
            self.connection.commit()
    ...
    def create_place(self, title, part_id):
        query = "INSERT INTO `amar2`.`tbl_places` (`title`, `section`, `order`) VALUES (%s, %s, 0);"
        values = (title, part_id)
        try:
            self.cursor.execute(query, values)
            self.connection.commit()
            query = "SELECT * FROM `amar2`.`tbl_places` where title=%s AND section=%s;"
            self.cursor.execute(query, values)
            result = self.cursor.fetchone()
            if result in [None, '', ()]:
    ...
    def update_place(self, id, new_name, part_id):
        query = "UPDATE `amar2`.`tbl_places` SET `title` = %s, `section` = %s WHERE (`id` = %s);"
    ...
    def delete_place(self, id):
        query = "DELETE FROM `amar2`.`tbl_places` WHERE (`id` = %s);"
        values = (id, )
        try:
            self.cursor.execute(query, values)
            self.connection.commit()
    ...
    def update_place_sort(self, id, order):
        query = "UPDATE `amar2`.`tbl_places` SET `order` = %s WHERE (`id` = %s);"
        values = (order, id)
        self.cursor.execute(query, values)
        self.connection.commit()
        return ("ok", 0)
    ...
    def get_part_by_id(self, id):
        query = "SELECT * FROM `amar2`.`tbl_sections` WHERE id=%s;"
        values = (id, )
        self.cursor.execute(query, values)
    ...
    def create_parameter(self, name, type, unit, default_value, variable_name, warning_lower_bound, warning_upper_bound, alarm_lower_bound, alarm_upper_bound, formula, part, place):
        query = "INSERT INTO `amar2`.`tbl_parameters` (`name`, `type`, `unit`, `default_value`, `variable_name`, `warning_lower_bound`, `warning_upper_bound`, `alarm_lower_bound`, `alarm_upper_bound`, `formula`, `section`, `place`, `order`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 0);"
    ...
    def update_parameter(self, name, type, unit, default_value, variable_name, warning_lower_bound, warning_upper_bound, alarm_lower_bound, alarm_upper_bound, formula, part, place):
        query = "UPDATE `amar2`.`tbl_parameters` SET `name` = %s, `type` = %s, `unit` = %s, `default_value` = %s, `variable_name` = %s, `warning_lower_bound` = %s, `warning_upper_bound` = %s, `alarm_lower_bound` = %s, `alarm_upper_bound` = %s, `formula` = %s, `section` = %s, `place` = %s WHERE (`variable_name` = %s);"
        part_id, part_title, part_sort = self.get_part_by_title(part)
        place_id, place_title, place_part_id, place_sort = self.get_place_by_title_and_part_id(place, part_id)
    ...
    def delete_parameter(self, id):
        query = "DELETE FROM `amar2`.`tbl_parameters` WHERE (`id` = %s);"
        values = (id, )
        try:
            self.cursor.execute(query, values)
            self.connection.commit()
    ...
    def get_all_parts(self):
        query = "SELECT `title`, `id` FROM `amar2`.`tbl_sections` ORDER BY `order`;"
        self.cursor.execute(query)
        all_parts = []
    ...
    def get_all_places_by_part_id(self, part_id):
        query = "SELECT `amar2`.`tbl_places`.`title`, `amar2`.`tbl_places`.`section` as `section_id`, `amar2`.`tbl_places`.`id`, `amar2`.`tbl_sections`.`title` as `section_title` FROM `amar2`.`tbl_places` join `amar2`.`tbl_sections` ON (`amar2`.`tbl_places`.`section`=`amar2`.`tbl_sections`.`id`) WHERE `amar2`.`tbl_places`.`section`=%s ORDER BY `amar2`.`tbl_places`.`order`;"
        values=(part_id, )
        self.cursor.execute(query, values)
        all_places = []
    ...
    def get_last_log_of_parameter_by_id(self, id):
        query = "SELECT `date` FROM `amar2`.`tbl_parameters_log` WHERE `parameter_id`=%s ORDER BY `date` DESC LIMIT 1;"
        values = (id, )
        self.cursor.execute(query, values)
        temp = self.cursor.fetchone()
        if temp == None:
    ...
    def get_all_parameters_of_this_part_and_place(self, part_id, place_id):
        query = "SELECT `amar2`.`tbl_parameters`.`section`, `place`, `name`, `variable_name`, `formula`, `type`, `default_value`, `unit`, `warning_lower_bound`, `warning_upper_bound`, `alarm_lower_bound`, `alarm_upper_bound`, `amar2`.`tbl_parameters`.`id`, `amar2`.`tbl_places`.`title` as `place_title`, `amar2`.`tbl_sections`.`title` as `section_title` FROM `amar2`.`tbl_parameters` join `amar2`.`tbl_places` ON (`amar2`.`tbl_parameters`.`place`=`amar2`.`tbl_places`.`id`) join `amar2`.`tbl_sections` ON (`amar2`.`tbl_parameters`.`section`=`amar2`.`tbl_sections`.`id`) WHERE `amar2`.`tbl_parameters`.`section`=%s AND `place`=%s ORDER BY `amar2`.`tbl_sections`.`order` ASC, `amar2`.`tbl_places`.`order` ASC, `amar2`.`tbl_parameters`.`order` ASC;"
    ...
    def create_parameter_log(self, value, workout, is_ok, date, parameter_id, user_id):
        query = "INSERT INTO `amar2`.`tbl_parameters_log` (`value`, `workout`, `is_ok`, `date`, `date_time_modified`, `parameter_id`, `user_id`) VALUES (%s, %s, %s, %s, %s, %s, %s);"
    ...
    def update_parameter_log(self, value, workout, is_ok, date, parameter_id, user_id):
        query = "SELECT `id` FROM `amar2`.`tbl_parameters_log` WHERE `parameter_id`=%s AND `date`<=%s ORDER BY `date` DESC LIMIT 1;"
        ...
        query = "UPDATE `amar2`.`tbl_parameters_log` SET `value` = %s, `workout` = %s, `is_ok` = %s, `date_time_modified` = %s, `user_id` = %s WHERE (`id` = %s);"
    ...
    def get_parameter_log_by_parameter_id_and_date(self, parameter_id, date):
        query = "SELECT `value`, `workout`, `is_ok`, `date`, `date_time_modified`, `parameter_id`, `user_id`, `amar2`.`tbl_parameters_log`.`id`, `amar2`.`tbl_users`.`name` as `users_name`, `amar2`.`tbl_users`.`surname` as `users_surname` FROM `amar2`.`tbl_parameters_log` join `amar2`.`tbl_users` ON (`amar2`.`tbl_parameters_log`.`user_id`=`amar2`.`tbl_users`.`id`)  WHERE `parameter_id`=%s AND `date`<=%s ORDER BY `date` DESC LIMIT 1;"
        values = (parameter_id, date)
        self.cursor.execute(query, values)
        temp = self.cursor.fetchone()
        if temp == None:
    ...
    def change_log_by_computer_id(self, log:ParameterLog):
        if log.type==PARAMETER_TYPES[2]: # برای محاسباتی ها، مقدار ولیو و ورک اوت با یک مقدار ثبت میشن
            query = "UPDATE `amar2`.`tbl_parameters_log` SET `value` = %s, `workout` = %s, `date_time_modified` = %s, `user_id` = %s WHERE (`id` = %s);"
            ...
            query = "UPDATE `amar2`.`tbl_parameters_log` SET `workout` = %s, `date_time_modified` = %s, `user_id` = %s WHERE (`id` = %s);"
    ...
    def get_previous_value_of_parameter_by_id_and_date(self, id, date):
        query = "SELECT `value` FROM `amar2`.`tbl_parameters_log` WHERE `parameter_id`=%s AND `date`<=%s ORDER BY `date` DESC LIMIT 2"
        values = (id, date)
        self.cursor.execute(query, values)
    ...
    def get_all_parameters_variable_names(self):
        query = "SELECT `variable_name` FROM `amar2`.`tbl_parameters` ORDER BY `order`;"
        self.cursor.execute(query)
        names = []
        for item in self.cursor.fetchall():
            names.append(item[0])
    ...
    def get_parameter_by_variable_name(self, variable_name): 
        query = "SELECT `section`, `place`, `name`, `variable_name`, `formula`, `type`, `default_value`, `unit`, `warning_lower_bound`, `warning_upper_bound`, `alarm_lower_bound`, `alarm_upper_bound`, `id` FROM `amar2`.`tbl_parameters` WHERE `variable_name`=%s;"
    ...
    def get_parameter_by_id(self, id):
        query = "SELECT `section`, `place`, `name`, `variable_name`, `formula`, `type`, `default_value`, `unit`, `warning_lower_bound`, `warning_upper_bound`, `alarm_lower_bound`, `alarm_upper_bound`, `id` FROM `amar2`.`tbl_parameters` WHERE `id`=%s;"
        values = (id, )
        self.cursor.execute(query, values)
        temp = self.cursor.fetchone()
        if temp == None:
    ...
    def get_current_value_of_parameter_by_variable_name(self, variable_name):
        query = "SELECT `id` FROM `amar2`.`tbl_parameters` WHERE `variable_name`=%s;"
        values = (variable_name, )
        self.cursor.execute(query, values)
        id = self.cursor.fetchone()[0]
        query = "SELECT `value` FROM `amar2`.`tbl_parameters_log` WHERE `parameter`=%s ORDER BY `date` DESC LIMIT 1;"
        values = (id, )
        self.cursor.execute(query, values)
    ...
    def change_parameters_order(self, id, order):
        query = "UPDATE `amar2`.`tbl_parameters` SET `order` = %s WHERE (`id` = %s);"
        values=(order, id)
        self.cursor.execute(query, values)
        self.connection.commit()
        return ("ok", 0)
    ...
    def get_all_parameters_current_value_and_workout(self, selected_date):
        ...
        query = "SELECT `amar2`.`tbl_parameters`.`id`, `variable_name`, `value`, `workout` FROM (`amar2`.`tbl_parameters` JOIN `amar2`.`tbl_parameters_max_date` ON (`amar2`.`tbl_parameters`.`id`=`amar2`.`tbl_parameters_max_date`.`parameter_id`)) JOIN `amar2`.`tbl_parameters_log` ON (`amar2`.`tbl_parameters`.`id`=`amar2`.`tbl_parameters_log`.`parameter_id` AND `amar2`.`tbl_parameters_max_date`.`max_date`=`amar2`.`tbl_parameters_log`.`date`);"
        self.cursor.execute(query)
        temp_dict = {}
        for item in self.cursor.fetchall():
            try:
    ...
    def get_all_parameters_formula(self):
        query = "SELECT `formula` FROM `amar2`.`tbl_parameters`;"
        self.cursor.execute(query)
        formulas = []
